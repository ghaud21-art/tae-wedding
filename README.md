...
## 4. 사진 올리기 (구글 드라이브 — 폴더에 넣기만 하면 자동 반영)

"태경님 청첩장 사진" 폴더 안에 섹션별 하위 폴더가 있습니다:

| 폴더 | 사이트에 표시되는 곳 |
| --- | --- |
| 메인 | 첫 화면 큰 사진 (1장, 여러 장이면 파일명 순 첫 번째) |
| 인터뷰 | 인터뷰 섹션 사진 (1장) |
| 갤러리 | 웨딩 갤러리 (전부, 파일명 순서대로) |
| 마무리 | 마지막 섹션 사진 (1장) |
| 게스트스냅 | 하객이 사이트에서 "사진 보내기"로 올린 사진이 자동으로 모이는 곳 |

**해당 폴더에 사진을 올리기만 하면 사이트에 자동으로 반영됩니다** (5번의 웹 앱 배포가 되어 있어야 동작). 갤러리 순서를 바꾸고 싶으면 파일명 앞에 `01_`, `02_` 같은 번호를 붙이세요.

웹 앱 배포 전에는 예전 방식(파일 ID를 시트에 붙여넣기)도 그대로 동작합니다: 파일을 열면 주소창의 `https://drive.google.com/file/d/`**`파일ID`**`/view`에서 ID를 복사해 "설정" 탭의 `heroImageId` 등이나 "갤러리" 탭에 넣으면 됩니다.

## 5. 웹 앱 배포 — 사진 자동 연동 + RSVP 저장 + 게스트스냅 수신 (한 번만)

이 배포 하나로 세 가지가 켜집니다: ① 드라이브 폴더 사진 자동 연동, ② 참석 여부가 "참석여부" 탭에 저장, ③ 하객이 보낸 사진이 "게스트스냅" 폴더에 저장.

1. Apps Script 편집기(1번에서 연 곳)에서 우측 상단 **[배포] → [새 배포]**.
2. 유형 선택 톱니바퀴 → **웹 앱** 선택.
3. "실행 계정"은 **나**, "액세스 권한을 가진 사용자"는 **"모든 사용자"** → **[배포]**.
...
※ 이후 Code.gs 코드를 고쳤을 때는 **[배포] → [배포 관리] → 연필 아이콘 → 버전: 새 버전 → [배포]**를 해야 반영됩니다.
